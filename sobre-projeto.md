...
## 🎯 Objetivo
O objetivo principal é permitir a comparação justa e científica de diversos modelos de Redes Neurais em Grafos (GNNs) utilizando datasets padrão do estado da arte (`METR-LA` e `PEMS-BAY`). O projeto padroniza todo o pipeline, desde o carregamento dos dados até a geração de relatórios consolidados.
...
- **`backbone/`**: Ferramentas para filtragem e extração de estruturas principais (backbones) de grafos, como *Disparity Filter* e *High Salience Skeleton*.
