...
from refat.bootstrap import ensure_legacy_project_on_path
from refat.config import RefatConfig
from refat.datasets import (
    DatasetGroup,
    infer_base_dataset_name,
    resolve_backbone_dataset_names,
    resolve_dataset_paths,
)
from refat.model_registry import build_param_grids, load_grid_search_registry


_LEGACY_RUNTIME: dict[str, object] | None = None
...
def _build_groups(config: RefatConfig) -> list[DatasetGroup]:
...
def _prepare_dataloaders(config: RefatConfig, *, data_path: Path, adj_path: Path):
    import numpy as np
...
def _run_group(
    *,
    ...
    _legacy_runtime()["set_results_root"](results_root)
...
def run_pipeline(config: RefatConfig, *, dry_run: bool = False) -> None:
