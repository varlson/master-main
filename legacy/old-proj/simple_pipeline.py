...
def _parse_dataset_names(raw_value: Any) -> list[str]:
    if raw_value is None:
        return ["pems-bay", "metr-la", "wikivital-mathematics"]

    if isinstance(raw_value, str):
        names = [item.strip() for item in raw_value.split(",") if item.strip()]
        return names or ["pems-bay", "metr-la", "wikivital-mathematics"]
...
def _infer_base_dataset_name(dataset_name: str) -> str | None:
    ...
    if dataset_name == "pems-bay" or dataset_name.startswith("pems-bay-"):
        return "pems-bay"
    if dataset_name == "wikivital-mathematics" or dataset_name.startswith("wikivital-mathematics-"):
        return "wikivital-mathematics"
    return None
...
def load_model_params(params_file: Path) -> dict[str, dict[str, Any]]:
    ...
    selected_models = [model_name for model_name in SUPPORTED_MODELS if model_name in model_payload]
    if not selected_models:
        raise ValueError(
            "Nenhum modelo valido foi encontrado no arquivo de parametros. "
            f"Modelos aceitos: {list(SUPPORTED_MODELS)}"
        )
    ...
    *,
    ...
    if model_name == "GraphWaveNet":
        ...
        model = DCRNN(
            **common_kwargs,
            k=int(params.get("k", 2)),
            use_scheduled_sampling=bool(params.get("use_scheduled_sampling", False)),
            teacher_forcing_ratio=float(params.get("teacher_forcing_ratio", 0.5)),
        )
        ...
        model = PatchSTG(
            **common_kwargs,
            ...
            patch_stride=int(params.get("patch_stride", 2)),
            num_layers=int(params.get("num_layers", 2)),
            num_heads=int(params.get("num_heads", 4)),
            ff_multiplier=int(params.get("ff_multiplier", 2)),
        )
...
def run_dataset_pipeline(
    *,
    ...
    experiments_data: list[dict[str, Any]] = []
    for model_name, model_params in params_by_model.items():
        try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pipeline simples para modelos de forecasting em grafos.")
    parser.add_argument(
        "--config",
...
def main() -> None:
    ...
    print(f"- MODELS: {list(params_by_model)}")
