...
if __name__ == "__main__":
    ...
    print("\nAplicando corte com alpha = 0.05...")
    report = df_filter.backbone_report(alpha=0.05)
    for k, v in report.items():
        print(f"  {k}: {v}")

    backbone_alpha = df_filter.filter_by_alpha(alpha=0.05, min_degree=1)
    print(f"\nBackbone (alpha=0.05): {len(backbone_alpha.nodes())} nós, {len(backbone_alpha.edges())} arestas")
